...
then
  ...
  docker run -v $(pwd)/output:/output -p 8009:8009 darkenvy/kiwix-serve ./kiwix-serve --port=8009 /output/`ls -t output | head -1`
elif [ $1 = 'stop' ]
then
  ...
  docker run -v $(pwd)/output:/output -p 8009:8009 darkenvy/kiwix-serve ./kiwix-serve --port=8009 /$CMD
fi
