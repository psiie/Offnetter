...
// pool.on("done", (job, message) => console.log("Job done:"));
pool.on("error", (job, error) => console.error("Job errored"));
pool.on("finished", () => {
  console.log("Everything done, shutting down the thread pool.");
  pool.killAll();
});

function poolProcessHtml({ html, zimList }, done) {
  let newHtml = html;
...
function modifyHtml(zimList) {
  function cleanSingleFile(file, callback) {
    ...
    fs.readFile(filePath, "utf8", (err, html) => {
      ...
      const job = pool.run(poolProcessHtml).send({ html, zimList });
      job.on("done", newHtml => saveFile(file, newHtml, callback));
    });
  }

  const zimListArr = Object.keys(zimList);
  const queue = async.queue(cleanSingleFile, THREADS); // Can only be 1 concurrency here
